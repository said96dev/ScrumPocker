...
const DeleteDialog = ({ openDelete, setOpenDelete, id }: DeleteDialog) => {
  ...
  return (
    <dialog open={openDelete} className='modal'>
      {' '}
      {/* Stelle sicher, dass open Attribute hier ist */}
      <div className='modal-box w-11/12 max-w-5xl'>
        ...
        <form method='dialog' className='flex gap-4'>
          <button
            onClick={() => setOpenDelete(false)}
            className='btn btn-base-100 mt-5 min-w-24 btn-neutral'
          >
            cancel
          </button>
          <button
            onClick={handleDelete}
            className='btn btn-error mt-5 min-w-24'
          >
            Delete
          </button>
        </form>
